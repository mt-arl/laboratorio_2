...
def consultar_nrc(materia):
    ...
    try:
        ...
        return {"status": "error", "mensaje": "Timeout: Servidor NRC no respondio"}
    except ConnectionRefusedError:
        return {"status": "error", "mensaje": "Error: Servidor NRC no disponible"}
    except Exception as e:
        return {"status": "error", "mensaje": f"Error consultando NRC: {str(e)}"}

def verificar_id_existe(id_est, materia=None):
    """Verifica si un ID y materia ya existen"""
    try:
        if not os.path.exists(ARCHIVO_CSV):
            return False, None
...
def agregar_calificacion(id_est, nombre, materia, calif):
    """Agrega una nueva calificación al CSV con validaciones y NRC"""
    try:
        # Validar NRC primero
...
def manejar_cliente(client_socket, addr):
    ...
    try:
        ...
        if data:
            ...
            print(f"Respuesta enviada a {addr}: {respuesta['status']}")
    except Exception as e:
        print(f"Error en hilo {threading.current_thread().name}: {e}")
        error_msg = {"status": "error", "mensaje": f"Error interno del servidor: {str(e)}"}
...
def main():
    ...
    print(f"Servidor concurrente escuchando en puerto {PUERTO}...")
